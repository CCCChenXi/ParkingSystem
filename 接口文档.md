...
无需参数，返回所有停车场（无 geo 筛选，不分页）。后端三级缓存：Caffeine(L1, 60s) → Redis(L2, 30min) → DB(L3)。
...
```
GET /api/orders?status=0&lastTimestamp=1712345678000&lastId=99&pageSize=10
```

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| status | number | 是 | 0/1/2/3 |
| lastTimestamp | number | 否 | 上一页最后一条 createTime(epoch ms)，首次不传 |
| lastId | number | 否 | 上一页最后一条 id，首次不传 |
...
|------|------|
| `data` | 当前页订单数组 |
| `nextTimestamp` | 下一页游标时间戳，下一页请求的 lastTimestamp |
| `nextId` | 下一页游标 ID，下一页请求的 lastId |
| `hasMore` | false 表示已无更多数据 |

**后端逻辑**
```sql
WHERE status = :status
  AND (createTime < :lastTimestamp OR (createTime = :lastTimestamp AND id < :lastId))
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `couponId` | number / null | 是 | 优惠券 ID，传 `null` 表示无折扣 |
