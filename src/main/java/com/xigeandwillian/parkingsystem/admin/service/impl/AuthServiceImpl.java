...
public class AuthServiceImpl implements AuthService {
    ...
    public Result login(LoginDTO loginDTO) {
        ...
        if (admin == null || !encryptedPassword.equals(admin.getPassword())) {
            ...
            if (count == 1) {
                if (redisService.expire(key, RedisConstant.Auth.ADMIN_LOGIN_ERROR_RESET_TTL_DAY, TimeUnit.DAYS) == null) {
                    log.error("设置计数过期失败");
                    throw new BusinessException(ResultConstant.INTERNAL_SERVER_ERROR, "系统繁忙，请稍后再试");
                }
            }
        ...
        redisService.set(RedisConstant.Auth.ADMIN_SESSION_PREFIX + admin.getId(), JSONUtil.toJsonStr(adminVO), RedisConstant.Auth.ADMIN_SESSION_TTL_HOUR, TimeUnit.HOURS);
