...
public class CouponDataProvider {
    ...
    private static final String LOCAL_KEY = RedisConstant.Coupon.AVAILABLE_KEY;
    ...
    private static final String AVAILABLE_LOCK_KEY = RedisConstant.Coupon.COUPON_REBUILD_LOCK_AVAILABLE;
    private static final String DETAIL_LOCK_KEY = RedisConstant.Coupon.COUPON_REBUILD_LOCK_DETAIL;
        ...
        try {
            ...
            for (Coupon c : seckillCoupons) {
                ...
                stringRedisTemplate.opsForValue().setIfAbsent(
                        ...
                        RedisConstant.Coupon.SECKILL_STOCK_TTL_DAY, TimeUnit.DAYS);
            }
    ...
    public void invalidateAvailable() {
        localCache.invalidate(LOCAL_KEY);
        try {
            stringRedisTemplate.delete(REDIS_KEY);
        } catch (Exception e) {
            log.warn("可用优惠券Redis缓存删除失败", e);
        }
        cacheInvalidateProducer.sendCacheInvalidate(LOCAL_KEY);
    }

    public void invalidateLocalAvailable() {
        localCache.invalidate(LOCAL_KEY);
    }
    ...
    private void rebuildDetailRedisCache(Map<Long, CouponDetailVO> map) {
        try {
            stringRedisTemplate.opsForValue().set(
                    ...
                    RedisConstant.Coupon.DETAIL_TTL_MIN,
                    TimeUnit.MINUTES);
        } catch (Exception e) {
    ...
    private CouponAvailableVO toVO(Coupon coupon) {
        return couponConverter.toAvailableVO(coupon);
    }

    private CouponDetailVO toDetailVO(Coupon coupon) {
        return couponConverter.toDetailVO(coupon);
    }
    ...
    private void rebuildRedisCache(List<CouponAvailableVO> data) {
        try {
            ...
            stringRedisTemplate.opsForValue()
                    .set(REDIS_KEY, json, RedisConstant.Coupon.AVAILABLE_TTL_MIN, TimeUnit.MINUTES);
        } catch (Exception e) {
    ...
    private void saveStaticToRedis(Long id, CouponDetailVO vo) {
        try {
            stringRedisTemplate.opsForValue().set(
                    ...
                    RedisConstant.Coupon.STATIC_TTL_MIN, TimeUnit.MINUTES);
        } catch (Exception e) {
    ...
    private void saveStaticNullToRedis(Long id) {
        try {
            stringRedisTemplate.opsForValue().set(
                    RedisConstant.Coupon.STATIC_KEY + id,
                    "",
                    RedisConstant.Coupon.STATIC_NULL_TTL_MIN, TimeUnit.MINUTES);
        } catch (Exception e) {
